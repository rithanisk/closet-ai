  ...
  return (
    <div className="page settings-page">
      ...
      <form className="settings-stack" onSubmit={submit}>
        ...
        <section className="settings-card"><div className="settings-title"><span><ShieldCheck size={18} /></span><div><h2>Data & privacy</h2><p>Your wardrobe is personal. It stays that way.</p></div></div><div className="settings-fields privacy-copy"><p>Original upload photos are discarded after extraction. Only approved item cutouts and wardrobe details are retained. If you create a twin in the fitting room, its reference photos and try-on looks are stored privately until you delete the twin. Images are never used to train shared models.</p></div></section>
        <div className="settings-save"><button className="button button-primary" type="submit">{saved ? 'Saved ✓' : 'Save settings'}</button></div>
      </form>
      <section className="danger-zone"><div><div className="eyebrow">Danger zone</div><h2>Delete account</h2><p>Permanently delete your account, wardrobe, and saved outfits.</p></div><button className="button button-danger-soft" onClick={onDeleteAccount}><Trash2 size={15} /> Delete account</button></section>
    </div>
  );
}
