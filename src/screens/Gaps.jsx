...
export function GapsScreen({ wardrobe, onToast, onUpload, onInspiration }) {
  ...
  useEffect(() => {
    let active = true;
    api('/api/gaps').then((data) => { if (active) setSuggestions(data.suggestions); }).catch((error) => onToast(error.message));
    return () => { active = false; };
  }, [onToast]);
  ...
  return (
    <div className="page gaps-page">
      <PageHeader
        ...
        action={<button className="button button-dark" onClick={analyze} disabled={busy || availableCount < 6}>{busy ? <LoadingLine>Analyzing…</LoadingLine> : <><RefreshCw size={15} /> {active.length ? 'Re-check gaps' : 'Find wardrobe gaps'}</>}</button>}
      />
      ...
      {availableCount < 6 ? (
        <EmptyState icon={ShoppingBag} title="Add a few more pieces first" body="Gap suggestions are grounded in real outfits, so they need at least six available pieces to work with." action={<button className="button button-dark" onClick={onUpload}>Upload photos</button>} />
      ) : !active.length && !busy ? (
        <EmptyState icon={Sparkles} title="No open suggestions" body="Run a gap check when you're thinking about shopping. We'll look for the one or two pieces that unlock the most looks, and skip anything you already have a close version of." action={<div className="header-actions"><button className="button button-dark" onClick={analyze}>Find wardrobe gaps</button><button className="button button-outline" onClick={onInspiration}>Tune with inspiration</button></div>} />
      ) : (
          ...
          <div className="wishlist-grid">
            {wishlist.map((entry) => (
              <article className="wishlist-card" key={entry.id}>
                ...
                <div className="wishlist-actions"><button className="button button-outline" onClick={() => update(entry.id, 'purchased')}><Check size={13} /> Bought it</button><button className="text-link" onClick={() => remove(entry.id)}>Remove</button></div>
              </article>
            ))}
          </div>
        </section>
      ...
      <div className="gap-main">
        ...
        {dismissing ? (
          ...
          <div className="gap-actions">
            <button className="button button-dark" onClick={() => onUpdate(entry.id, 'wishlist')}><Bookmark size={14} /> Save to wishlist</button>
            <button className="button button-outline" onClick={() => onUpdate(entry.id, 'owned')}><CircleCheck size={14} /> I already own this</button>
